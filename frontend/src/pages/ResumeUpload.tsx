...
import { Upload, Loader2, CheckCircle2, FileText, Sparkles, AlertCircle, ArrowRight, Zap, RefreshCw } from 'lucide-react';
...
const ResumeUpload = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      // Send user ID to associate parsed skills with user account
      const userId = user?.id || 'demo-user-id';
      const data = await resumeService.uploadResume(file, userId);
      setResult(data);
    } catch (e: any) {
      console.error('Resume upload failed:', e);
      const msg = e.response?.data?.detail || e.message || 'Failed to analyze resume. Please make sure all microservices are running.';
      setError(msg);
    } finally {
      setLoading(false);
    }
    ...
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <div className="glass-panel p-8 relative overflow-hidden flex flex-col md:flex-row items-start md:items-center justify-between gap-6 border-l-4 border-cyan-400">
        <div className="space-y-2">
          <div className="flex items-center gap-2 text-cyan-400 text-xs font-semibold uppercase tracking-wider">
            <Sparkles className="w-4 h-4" /> Python FastAPI Resume Microservice
          </div>
          <h1 className="text-3xl font-extrabold">Upload & Analyze Resume</h1>
          <p className="text-sm text-gray-400 max-w-xl">
            Upload your resume document. Our Python microservice extracts technical skills without false positives, calculates ATS optimization score, and recommends targeted improvements.
          </p>
        </div>

        <div className="glass-panel px-4 py-2 text-xs font-bold text-cyan-400 flex items-center gap-2">
          <FileText className="w-4 h-4" /> PDF, DOCX, TXT, RTF Supported
        </div>
      </div>
      ...
      <div className="glass-panel p-10 text-center space-y-6 border-2 border-dashed border-gray-700/80 hover:border-cyan-500/60 transition-all rounded-2xl">
        ...
        <div className="space-y-2 max-w-md mx-auto">
          <h3 className="text-lg font-bold">Choose your CV document</h3>
          <p className="text-xs text-gray-400">Supported formats: PDF, DOCX, DOC, TXT, RTF (Up to 10MB)</p>
          <input
            type="file"
        ...
        {error && (
          <div className="p-4 rounded-xl bg-red-500/10 border border-red-500/30 text-xs text-red-400 font-semibold max-w-md mx-auto flex items-center gap-2 justify-center">
        ...
        <div className="glass-panel p-8 space-y-8 animate-fade-in border border-cyan-500/30">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-b border-gray-800 pb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-bold">Resume Parsed Successfully</h2>
                <p className="text-xs text-gray-400">Extracted by Python Resume Microservice</p>
              </div>
            </div>
            ...
            >
              <Zap className="w-4 h-4" /> View AI Job Matches <ArrowRight className="w-4 h-4" />
            </Link>
          </div>
          ...
          <div className="glass-card p-6 flex flex-col md:flex-row items-center justify-between gap-6 border border-cyan-500/20">
            <div className="space-y-1 text-center md:text-left">
              ...
              <p className="text-xs text-gray-400">
                {result.atsScore >= 80 ? '🎯 Excellent score! Highly optimized for recruiters.' : '💡 Good start. Follow suggestions below to increase match score.'}
              </p>
            </div>
          ...
          <div className="space-y-3">
            <h3 className="font-bold text-sm uppercase tracking-wider text-cyan-400">
              Genuine Extracted Skills ({result.skills?.length || 0})
            </h3>
            {(!result.skills || result.skills.length === 0) ? (
              <p className="text-xs text-gray-400 italic">No technical skills detected in document. Try adding specific tools like Python, React, SQL, etc.</p>
            ) : (
          ...
          <div className="grid md:grid-cols-2 gap-6 text-xs">
            <div className="p-5 rounded-2xl bg-green-500/5 border border-green-500/20 space-y-3">
              <h4 className="font-bold text-green-400 text-sm flex items-center gap-1.5">
                <CheckCircle2 className="w-4 h-4" /> Real Profile Strengths
              </h4>
            ...
            <div className="p-5 rounded-2xl bg-amber-500/5 border border-amber-500/20 space-y-3">
              <h4 className="font-bold text-amber-400 text-sm flex items-center gap-1.5">
                <AlertCircle className="w-4 h-4" /> Personalized Improvements
              </h4>
              <ul className="space-y-2 text-gray-300">
